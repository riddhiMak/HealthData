//
//  HealthTypeTableViewController.swift
//  HealthData
//
...
class HealthTypeTableViewController: UIViewController {
    ...
    let viewModel = HealthTypeViewModel()
...
extension HealthTypeTableViewController: HealthStoreManagerInjectable {
...
extension HealthTypeTableViewController:UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.recordTypesModel?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "HealthTypeTableViewCell") as? HealthTypeTableViewCell, let model = viewModel.recordTypesModel?[indexPath.row] {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let cell = tableView.cellForRow(at: indexPath) as? HealthTypeTableViewCell,
            let type = cell.cellModel?.type {
...
extension HealthTypeTableViewController {
